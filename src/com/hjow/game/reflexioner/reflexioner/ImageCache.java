...
public class ImageCache
{
    ...
    public static void prepareImage(Setting sets)
    {
        ...
        if(st == null) st = Setting.load();
        String path = st.getDefaultPath() + File.separator + "assets";
    ...
    public static List<String> needFiles(Setting st, boolean descriptions)
    {
        ...
        if(descriptions) lists.add(st.trans("도형을 대체해 이미지로 적과 함선, 미사일 등을 표시하려면 이미지 파일이 필요합니다."));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("다음은 필요한 이미지 파일 목록입니다. 다 있을 필요는 없으며, 각 항목 당 하나씩만 있으면 됩니다."));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("파일 이름 뿐만 아니라 경로까지 포함해 보여집니다. 해당 경로에 해당 이름으로 이미지 파일을 넣어야 한다는 뜻입니다."));
        if(descriptions) lists.add("");
        ...
        if(descriptions) lists.add(st.trans("기본적인 적을 대신할 이미지 파일로 사용됩니다."));
        ...
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("기본적인 적을 대신할 이미지 파일로 사용됩니다. (180도 뒤집힌 이미지)"));
        ...
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("향상된 적을 대신할 이미지 파일로 사용됩니다."));
        keyname = new BigEnemy().getEnemyName();
        lists.add(RunManager.r65279(path + keyname + ".png"));
        lists.add(RunManager.r65279(path + "enemy_" + keyname + ".jpg"));
        lists.add(RunManager.r65279(path + "enemy_" + "default" + ".png"));
        lists.add(RunManager.r65279(path + "enemy_" + "default" + ".jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("향상된 적을 대신할 이미지 파일로 사용됩니다. (180도 뒤집힌 이미지)"));
        lists.add(RunManager.r65279(path + "r_" + keyname + ".png"));
        lists.add(RunManager.r65279(path + "r_" + "enemy_" + keyname + ".jpg"));
        lists.add(RunManager.r65279(path + "r_" + "enemy_" + "default" + ".png"));
        lists.add(RunManager.r65279(path + "r_" + "enemy_" + "default" + ".jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("큰 적을 대신할 이미지 파일로 사용됩니다."));
        keyname = new Boss().getEnemyName();
        lists.add(RunManager.r65279(path + keyname + ".png"));
        lists.add(RunManager.r65279(path + "enemy_" + keyname + ".jpg"));
        lists.add(RunManager.r65279(path + "enemy_" + "default" + ".png"));
        lists.add(RunManager.r65279(path + "enemy_" + "default" + ".jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("큰 적을 대신할 이미지 파일로 사용됩니다. (180도 뒤집힌 이미지)"));
        lists.add(RunManager.r65279(path + "r_" + keyname + ".png"));
        lists.add(RunManager.r65279(path + "r_" + "enemy_" + keyname + ".jpg"));
        lists.add(RunManager.r65279(path + "r_" + "enemy_" + "default" + ".png"));
        lists.add(RunManager.r65279(path + "r_" + "enemy_" + "default" + ".jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("적이 발사하는 기본적인 발사체를 대신할 이미지 파일로 사용됩니다."));
        Missile missile = new Missile();
        missile.setOwner(0);
        keyname = missile.getMissileName();
        ...
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("플레이어의 함선이 발사하는 기본적인 발사체를 대신할 이미지 파일로 사용됩니다."));
        missile = new Missile();
        missile.setOwner(-1);
        keyname = missile.getMissileName();
        lists.add(RunManager.r65279(path + "missile_" + keyname + ".png"));
        lists.add(RunManager.r65279(path + "missile_" + keyname + ".jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("적이 발사하는 유도 발사체를 대신할 이미지 파일로 사용됩니다."));
        ...
        lists.add(RunManager.r65279(path + "missile_" + keyname + ".jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("플레이어의 함선이 발사하는 유도 발사체를 대신할 이미지 파일로 사용됩니다."));
        missile = new GuidedMissile();
        missile.setOwner(-1);
        keyname = missile.getMissileName();
        lists.add(RunManager.r65279(path + "missile_" + keyname + ".png"));
        lists.add(RunManager.r65279(path + "missile_" + keyname + ".jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("적이 발사하는 강화된 발사체를 대신할 이미지 파일로 사용됩니다."));
        ...
        lists.add(RunManager.r65279(path + "missile_" + keyname + ".jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("플레이어의 함선이 발사하는 강화된 발사체를 대신할 이미지 파일로 사용됩니다."));
        missile = new SuperMissile();
        missile.setOwner(-1);
        keyname = missile.getMissileName();
        lists.add(RunManager.r65279(path + "missile_" + keyname + ".png"));
        lists.add(RunManager.r65279(path + "missile_" + keyname + ".jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("적이 발사하는 반사성 발사체를 대신할 이미지 파일로 사용됩니다."));
        ...
        lists.add(RunManager.r65279(path + "missile_" + keyname + ".jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("플레이어의 함선이 발사하는 반사성 발사체를 대신할 이미지 파일로 사용됩니다."));
        missile = new ReflexMissile();
        missile.setOwner(-1);
        keyname = missile.getMissileName();
        lists.add(RunManager.r65279(path + "missile_" + keyname + ".png"));
        lists.add(RunManager.r65279(path + "missile_" + keyname + ".jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("적이 발사하는 방향성 발사체를 대신할 이미지 파일로 사용됩니다."));
        ...
        lists.add(RunManager.r65279(path + "missile_" + keyname + ".jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("플레이어의 함선이 발사하는 방향성 발사체를 대신할 이미지 파일로 사용됩니다."));
        missile = new DirectMissile();
        missile.setOwner(-1);
        keyname = missile.getMissileName();
        lists.add(RunManager.r65279(path + "missile_" + keyname + ".png"));
        lists.add(RunManager.r65279(path + "missile_" + keyname + ".jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("적이 발사하는 빔을 대신할 이미지 파일로 사용됩니다."));
        missile = new Beam();
        missile.setOwner(0);
        keyname = missile.getMissileName();
        lists.add(RunManager.r65279(path + "enemy_missile_" + keyname + ".png"));
        ...
        lists.add(RunManager.r65279(path + "missile_" + keyname + ".jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("플레이어의 함선이 발사하는 빔을 대신할 이미지 파일로 사용됩니다."));
        missile = new Beam();
        missile.setOwner(-1);
        keyname = missile.getMissileName();
        lists.add(RunManager.r65279(path + "missile_" + keyname + ".png"));
        lists.add(RunManager.r65279(path + "missile_" + keyname + ".jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("적의 공격으로 발생한 폭발을 대신할 이미지 파일로 사용됩니다."));
        OvalBoom boom = new OvalBoom();
        boom.setMaker(0);
        keyname = boom.getBoomName(true);
        String keyname2 = boom.getBoomName(false);
        ...
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("플레이어의 함선의 공격으로 발생한 폭발을 대신할 이미지 파일로 사용됩니다."));
        boom = new OvalBoom();
        boom.setMaker(-1);
        keyname = boom.getBoomName(true);
        ...
        lists.add(RunManager.r65279(path + "boom_" + "default" + ".jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("아이템을 획득할 때 발생하는 효과를 대신할 이미지 파일로 사용됩니다."));
        ...
        lists.add(RunManager.r65279(path + "boom_" + "default" + ".jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("배경 이미지로 사용됩니다."));        
        lists.add(RunManager.r65279(path + "background.png"));
        lists.add(RunManager.r65279(path + "background.jpg"));
        if(descriptions) lists.add("");
        if(descriptions) lists.add(st.trans("배경에 지나가는 별을 대신하는 이미지로 사용됩니다."));
        ...
        for(int i=0; i<ships.size(); i++)
        {
            ...
            {
                if(descriptions) lists.add("");
                if(descriptions) lists.add(st.trans("플레이어의 함선 " + ships.get(i) + " 의 " + String.valueOf(j) + " 번째 무기가 장착된 모습을 대체하는 이미지로 사용됩니다."));
